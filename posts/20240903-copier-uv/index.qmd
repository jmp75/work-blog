...
Tried several versions, but had issues down the track. Maybe works only if the specified versions have been indeed been installed via `pyenv`. Keep in log notes for now ans see if I need to sanitise for blog post.
...
Rather than `make setup` do the steps for didactic purposes:
